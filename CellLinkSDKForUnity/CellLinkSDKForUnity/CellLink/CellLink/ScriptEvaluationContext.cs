...
namespace CellLink
{
	...
	public class ScriptEvaluationContext
	{
		...
		{
			ScriptVerify = CellLink.ScriptVerify.Standard;
			SigHash = CellLink.SigHash.Undefined;
				...
				while((opcode = script.Read()) != null)
				{
					...
					else if(fExec || (OpcodeType.OP_IF <= opcode.Code && opcode.Code <= OpcodeType.OP_ENDIF))
					{
						switch(opcode.Code)
						{
                            ...
                            case OpcodeType.OP_PUB_CONTRACT:
                            case OpcodeType.OP_TRANS_CONTRACT:
                                {
                                    // do nothing
                                    break;
                                }
...
0xDF,0xE9,0x2F,0x46,0x68,0x1B,0x20,0xA0
		...
		bool CheckSig(byte[] vchSig, byte[] vchPubKey, Script scriptCode, TransactionChecker checker, int sigversion)
		{
			...
			if(!pubkey.Verify(sighash, scriptSig.Signature))
			{
				...
				var R = 4;
				var S = 6 + nLenR;
				var newS = new CellLink.BouncyCastle.Math.BigInteger(1, vchSig, S, nLenS);
				var newR = new CellLink.BouncyCastle.Math.BigInteger(1, vchSig, R, nLenR);
		...
		{
			if(SigHash == CellLink.SigHash.Undefined)
				return true;
