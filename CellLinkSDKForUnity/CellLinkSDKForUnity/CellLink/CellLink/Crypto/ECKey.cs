using CellLink.BouncyCastle.Asn1.X9;
using CellLink.BouncyCastle.Crypto.Parameters;
using CellLink.BouncyCastle.Crypto.Signers;
using CellLink.BouncyCastle.Math;
using CellLink.BouncyCastle.Math.EC;
using System;
using CellLink.BouncyCastle.Math.EC.Custom.Sec;

namespace CellLink.Crypto
{
	internal class ECKey
	{
		...
		static ECKey()
		{
			_Secp256k1 = CellLink.BouncyCastle.Crypto.EC.CustomNamedCurves.Secp256k1;
		...
		{
			if(isPrivate)
				_Key = new ECPrivateKeyParameters(new CellLink.BouncyCastle.Math.BigInteger(1, vch), DomainParameter);
			else
			{
		...
		public static ECKey RecoverFromSignature(int recId, ECDSASignature sig, uint256 message, bool compressed)
		{
			...
			var n = curve.N;
			var i = CellLink.BouncyCastle.Math.BigInteger.ValueOf((long)recId / 2);
			...
			var e = new CellLink.BouncyCastle.Math.BigInteger(1, message.ToBytes());
			...
			var eInv = CellLink.BouncyCastle.Math.BigInteger.Zero.Subtract(e).Mod(n);
		...
		private static ECPoint DecompressKey(CellLink.BouncyCastle.Math.BigInteger xBN, bool yBit)
		{
