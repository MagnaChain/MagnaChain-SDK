...
namespace CellLink.DataEncoders
{
	public class HexEncoder : DataEncoder
	{
        ...
        public static string HexStr(string strInput, bool bSpaces = false)
        {
            ...
            StringBuilder sbK = new StringBuilder(256);
            ...
            {
                ucVal = (byte)strInput[i];
                if ( bSpaces && i != 0)
                {
                    sbK.Append(' ');
                }
                sbK.Append(hexmap[ucVal >> 4]);
                sbK.Append(hexmap[ucVal & 15]);
            }
            return sbK.ToString();
