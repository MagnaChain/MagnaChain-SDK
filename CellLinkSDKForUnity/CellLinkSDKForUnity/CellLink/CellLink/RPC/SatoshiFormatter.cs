		...
		protected override void WriteTransaction(JsonTextWriter writer, Transaction tx)
		{
			...
			foreach(var txout in tx.Outputs)
			{
				...
				if(destinations[0] == null)
				{
                    ...
                    IEnumerable<KeyId> itKI = txout.ScriptPubKey.GetDestinationPublicKeys()
                                                        .Select(p => p.Hash);

                    List<KeyId> listKI = itKI.ToList<KeyId>();
                    destinations.Clear();

                    int k;

                    for ( k = 0; k < listKI.Count; k++ )
                    {
                        destinations.Add(listKI[k]);
                    }
                }
