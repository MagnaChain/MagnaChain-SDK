using CellLink.BuilderExtensions;
using CellLink.Crypto;
using CellLink.DataEncoders;
using CellLink.OpenAsset;
using CellLink.Policy;
using CellLink.Stealth;
using System;
...
using Builder = System.Func<CellLink.TransactionBuilder.TransactionBuildingContext, CellLink.IMoney>;
...
namespace CellLink
{
	...
	/// <summary>
	/// A class for building and signing all sort of transactions easily (http://www.codeproject.com/Articles/835098/CellLink-Build-Them-All)
	/// </summary>
	public class TransactionBuilder
	{
		...
		private void InitExtensions()
		{
            ...
            Extensions.Add(new ContractBuilderExtension());
		}
		...
		/// <exception cref="CellLink.NotEnoughFundsException">Not enough funds are available</exception>
		...
		public Transaction SignTransactionInPlace(Transaction transaction, SigHash sigHash)
		{
            ...
            if (transaction.IsSmartContract)
            {
                uint256 hashSign = Script.SignatureHashForContract(transaction);
                //hashSign = new uint256("fbfd681947f9178205065f85c6eaa8f964a5dcad1d126c38258cfc1369cfc4f0");
                //Console.WriteLine("contract sign hash : " + hashSign.ToString());
                Key key = FindKey(ctx, transaction.ContractSender.ScriptPubKey);
                TransactionSignature ts = key.Sign(hashSign, SigHash.All);
                transaction.ContractScriptSig = new Script( Op.GetPushOp(ts.ToBytes()) );
            }
			return transaction;
		}
