...
using CellLink.BouncyCastle.Asn1.Sec;
...
using Xunit;
using CellLink.BouncyCastle.Asn1;

namespace CellLink.Tests
{
   ...
   s = 1313A2E03F5412DDB296A22E2C455335545672D9F");
		...
		private Func<BouncyCastle.Crypto.IDigest> GetHash(string hash)
		{
			if(hash.Equals("SHA-256", StringComparison.OrdinalIgnoreCase))
				return () => new CellLink.BouncyCastle.Crypto.Digests.Sha256Digest();

			if(hash.Equals("SHA-1", StringComparison.OrdinalIgnoreCase))
				return () => new CellLink.BouncyCastle.Crypto.Digests.Sha1Digest();

			if(hash.Equals("SHA-224", StringComparison.OrdinalIgnoreCase))
				return () => new CellLink.BouncyCastle.Crypto.Digests.Sha224Digest();

			if(hash.Equals("SHA-384", StringComparison.OrdinalIgnoreCase))
				return () => new CellLink.BouncyCastle.Crypto.Digests.Sha384Digest();

			if(hash.Equals("SHA-512", StringComparison.OrdinalIgnoreCase))
				return () => new CellLink.BouncyCastle.Crypto.Digests.Sha512Digest();
