...
public class Role : MonoBehaviour
{
    ...
    private const string SAVE_ADDRESS_NAME_PREFIX = "CellLink_TiG_";

    public void Initialize(IxCellLinkBridge kCLB, string strMasterKey, string strAddress, string strDestAddress, Role kOther, int iID)
    {
        m_kCLB = kCLB;
        m_kOtherRole = kOther;
        m_iID = iID;
    ...
    public void OnClickTransfer()
    {
        ...
        IxCellLinkBridge.ON_TRANSFER_DONE fnTransferDone = delegate (RPCError kError, string strTxid)
        {
            if ( kError != null )
            {
    ...
    private void UpdateBalance()
    {
        ...
        m_fUpdateTick = -1.0f;
        IxCellLinkBridge.ON_GET_BALANCE fnBalance = delegate (float fBalance)
        {
